...
function getRoomRate(event) {
  ...
  let price = 0;
  let roomTypeSelected = document.querySelector(
    "input[name='radioRoomField']:checked"
  ).value;
  ...
  const selectedDiscount = document.querySelector(
    "input[name='radioDiscountField']:checked"
  );
  ...
  displayResults(discount, discountRoomCost, taxCost, totalCost);
}

function displayResults(discount, discountRoomCost, taxCost, totalCost) {
